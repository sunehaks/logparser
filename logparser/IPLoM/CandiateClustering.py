...
class LogParser:
    ...
    def Step1(self):
        ...
        self.df_log = self.log_to_dataframe(os.path.join(os.path.dirname(self.para.path), self.logname), regex, headers, self.para.logformat)
        lineCount = 1
...
#        eventDf.loc[:,['EventTemplate','Occurances']].
        file_name = 'output.log'
        with open(os.path.join(self.para.savePath, file_name),'w') as outfile:
            eventDf.to_string(outfile, columns = ['EventTemplate'], index= False, header = False)
    """
